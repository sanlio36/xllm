  ...
  if (sequence_params_.enable_schedule_overlap && token_id < 0) {
    ...
    if (::xllm::ExecutionConfig::get_instance().debug_log_dp_mtp_overlap()) {
      LOG(INFO) << "[DP_MTP_TOKEN_ACCT] append_token_fake_skip token_id="
                << token_id << ", request_id=" << request_id_
  ...
  if (token_offset > 0) {
    ...
    if (kv_state_.current_max_tokens_capacity() == 0) {
      ...
      if (::xllm::ExecutionConfig::get_instance()
        ...
        LOG(INFO) << "[DP_MTP_TOKEN_ACCT] "
                     "update_last_step_token_skip_no_capacity token_id="
                  << token.id << ", token_offset=" << token_offset
                  << ", request_id=" << request_id_
...
bool Sequence::finished() const {
  ...
  const Slice<int32_t> valid_tokens = tokens();
#if defined(USE_NPU)
  if (::xllm::ExecutionConfig::get_instance().debug_log_dp_mtp_overlap()) {
    ...
    LOG(INFO) << "[DP_MTP_TOKEN_ACCT] finished_check valid_tokens_count="
              << valid_tokens.size() << ", first_tokens=["
              << token_preview.str() << "], request_id=" << request_id_
              << ", cur_generated_token_idx=" << cur_generated_token_idx_
              << ", last_valid_token_id="
