...
ModelOutput AclGraph::replay(CausalLM* model,
  ...
  if (debug_graph_inputs) {
    LOG(INFO) << "[MTP_GRAPH_DEBUG] replay_inputs"
              << ", graph=" << this << ", persistent_param="
              << &persistent_param_ << ", bucket_tokens=" << num_tokens_
              << ", actual_tokens=" << actual_num_tokens
...
ModelOutput AclGraphExecutorImpl::run(const torch::Tensor& tokens,
  ...
  if (debug_graph_inputs) {
    LOG(INFO) << "[MTP_GRAPH_DEBUG] select"
              ...
              << ", graph_num_tokens=" << max_local_num_tokens
              << ", bucket_tokens=" << bucket_num_tokens
              << ", actual_tokens=" << n_tokens
              << ", local_batch_size=" << local_batch_size
              << ", global_batch_size=" << params_single.meta.num_sequences
  ...
  int32_t prepare_slot = -1;
  {
    ...
    if (debug_graph_inputs) {
      LOG(INFO) << "[MTP_GRAPH_DEBUG] prepare_selected"
                << ", slot=" << prepare_slot << ", graph=" << graph.get();
    }
#endif
  }
#if defined(USE_NPU)
