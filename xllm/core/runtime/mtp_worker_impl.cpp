...
std::optional<ForwardOutput> MTPWorkerImpl::step_empty(
  ...
  if (!pending_draft_context_.outputs.empty() &&
  ...
  if (!input.input_params.meta.batch_forward_type.is_decode()) {
    ...
    if (use_prelaunched_first_draft) {
      draft_outputs.emplace_back(
          std::move(pending_draft_context_.outputs.front()));
      draft_extend_prepared =
          std::move(pending_draft_context_.prepared_inputs.front());
    } else {
    ...
    for (int32_t i = 1; i < options_.num_speculative_tokens(); ++i) {
      if (use_prelaunched_first_draft &&
          static_cast<size_t>(i) < pending_draft_context_.outputs.size()) {
        draft_outputs.emplace_back(
            std::move(pending_draft_context_.outputs[static_cast<size_t>(i)]));
        draft_step_prepared[i] = std::move(
            pending_draft_context_.prepared_inputs[static_cast<size_t>(i)]);
      } else {
        draft_outputs.emplace_back(
    ...
    if (prelaunch_next_first_draft) {
      ...
      if (combined_draft_execution_path_ ==
        ...
        submit_pending_empty_followup_drafts(
            input, options_.num_speculative_tokens());
      }
...
std::optional<ForwardOutput> MTPWorkerImpl::step_decode(
  ...
  if (use_continuous_dsa_drafts && !all_drafts_prelaunched) {
    ...
    const ForwardInput& combined_draft_input =
        use_prelaunched_first_draft
            ? pending_draft_context_.prepared_inputs.front()
                                    : current_draft_input;
  ...
  for (int32_t draft_idx = 0; draft_idx < num_speculative_tokens; ++draft_idx) {
    ...
    if (use_prelaunched_first_draft &&
      ...
      draft_prepared[draft_idx] = std::move(
          pending_draft_context_.prepared_inputs[static_cast<size_t>(draft_idx)]);
        ...
        draft_prepared[draft_idx].sampling_params;
    {
      ...
      if (!pending_output_consumed && should_broadcast_spec_tokens(
              parallel_args_,
              get_optimization_config().enable_spec_token_broadcast,
...
void MTPWorkerImpl::submit_pending_first_draft(
  ...
  pending_draft_context_.outputs.emplace_back(
      run_llm_no_sync_impl(*draft_impl_,
                           draft_input,
                           *compute_stream_,
                           *compute_stream_,
      ...
      << "failed to prelaunch next MTP first draft";
  {
    ...
    SampleOutput& sample_output =
        pending_draft_context_.outputs.back().sample_output;
    if (should_broadcast_spec_tokens(
            parallel_args_,
            get_optimization_config().enable_spec_token_broadcast,
            all_greedy_sample)) {
...
void MTPWorkerImpl::submit_pending_followup_drafts(
    ...
    int32_t num_drafts) {
  CHECK(!pending_draft_context_.outputs.empty());
  CHECK_EQ(pending_draft_context_.outputs.size(), 1U);
  CHECK_EQ(pending_draft_context_.prepared_inputs.size(), 1U);
  ...
  for (int32_t draft_idx = 1; draft_idx < num_drafts; ++draft_idx) {
    ...
    if (layer::is_mtp_dsa_topk_reuse_enabled(
      ...
      next_input.input_params.mtp_topk_state =
          pending_draft_context_.outputs.back().mtp_topk_state;
    }
    ...
    pending_draft_context_.outputs.emplace_back(std::move(output.value()));
    SampleOutput& sample_output =
        pending_draft_context_.outputs.back().sample_output;
    if (should_broadcast_spec_tokens(
            parallel_args_,
...
bool MTPWorkerImpl::pending_draft_context_matches(
    const ForwardInput& input) const {
  return !pending_draft_context_.outputs.empty() &&
         pending_draft_context_.embedding_ids ==
             input.input_params.embedding.embedding_ids &&
         pending_draft_context_.request_ids ==
             input.input_params.embedding.request_ids &&
