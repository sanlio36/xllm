...
class W8A8DynamicLinear(nn.Module):
    ...
    def forward(
        ...
        # aclnnQuantMatmulWeightNz requires pertoken_scale to be 1-D. A 3-D input
        # ``[batch, seq, hidden]`` makes ``dynamic_quant`` produce a 2-D pertoken
        # -> crash. Flatten to ``[-1, in_features]`` (pertoken becomes 1-D), run
        # the fused matmul, then reshape the output back.
        leading = x.shape[:-1]
        x_flat = x.reshape(-1, x.shape[-1])
        x_int8, pertoken = kernels.dynamic_quant(x_flat)
        out = self.forward_quantized(x_int8, pertoken)
